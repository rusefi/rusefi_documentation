...
Saturday Feb 10 2024 status: With the stock ECU, as a man in the middle, I substitute all torque numbers from the ECU with TPS-based approximation (and TPS does not really mean thus torque does not really mean)
...
Tomorrow I will take steps back and record traces of the same exact experiments with different versions of the script, not just 'no substitution' and 'full substitution'.

As of Feb 2024 it has been _years_ since I've started to work on and off on getting an open source ECU to communicate with a transmission control unit via CANbus. So far I have not accomplished the goal which makes it a great point to write an article on the subject!

I have a [2006 Volkswagen Passat 2.0 Turbo GDI PG35 non-DSG automatic complete running and driving test mule](https://rusefi.com/forum/viewtopic.php?f=2&t=1631) idling on rusEFI open source ECU. Attempting to engage transmission while posting only minimal dashboard CANbus messages causes something violent to happen and the engine would usually stall, as in [this video](https://youtu.be/rTobt4l-iEs).

The original ECU which I am looking to replace for no good reason is BOSCH MED9.1. Conveniently, the relevant Bosch Funktionsrahmen Ea827 TSI is available on the internet. CAN bus traffic seems to mostly align with [the DBC file](https://github.com/commaai/opendbc/blob/master/vw_golf_mk4.dbc). I've used Google Translate to translate some of interesting field descriptions; those are already merged into the file.

## Toolset

* PCAN USB dongle to record CANbus traces
* A few Proteus units for Man-in-the-middle research
* VAG diagnostics cable to read and reset error codes
* I've also created [my own suite](https://github.com/rusefi/can-log-tools/) of CANbus trace processing utilities.

## First experiments

First let's grab a trace and a snapshot of CANbus traffic while engine is not running, both with the OEM ECU and with the OEM ECU disconnected. We get https://github.com/rusefi/rusefi_documentation/commit/722e81d2b0dd2b1f9661a3e9fd6383087ba4418a#diff-457155f3530fe11dbca1083aa1860f4e3cccc8f2d8b78d81634def58fbc38d9f
...
I am going to use the [man-in-the-middle](https://en.wikipedia.org/wiki/Man-in-the-middle_attack) approach, meaning that I want to drop the CAN bus wire coming into TCU, install my Proteus and by means of Lua scripting alter the flow between the vehicle and the TCU. By doing that I hope to understand what the TCU cares about and what it doesn't care about while running the OEM ECU in order to later apply that knowledge while making my own ECU cooperate with same TCU.

On this Passat, the TCU is located under the fender. I've cut and extended the wires. The Proteus now sits between the TCU and the rest of the vehicle.

The first script would just pass all messages and just print ``isShiftActive``
...
Well, the TCU did care. Let's drop something else, let's drop MOTOR_5 0x480.
