...
This page contains general information about how to hook up to rusEFI hardware. It's not intended to be a guide to a specific hardware variant - rather, how to figure out what you're going to need to run the different kinds of hardware you'd find in an engine bay.
...
Most hardware variants support only logic level drive for coils. This means GM LSx, IGN1A, Denso coil stick, etc. (others may too - these are just examples) that have built-in ignitors will work. You should consult the documentation for both the coils and the rusEFI hardware you are using to determine whether you can directly wire the coils or an external ignitor/something else is required. Note: coil outputs are among the *least* robust (in terms of tolerating shorts, excessive voltages, etc.) on many hardware variants, including Proteus. Double-check your wiring! uaEFI supports IGBT expansion for dumb coils, kindly read the uaEFI manual for more information.

Like injectors, all coil outputs are interchangeable. Coil outputs do not need to be used sequentially, and firing order firing and phasing can be adjusted in software. Wasted spark (one coil fires two cylinders) and distributor (one coil mechanically multiplexed among all cylinders) modes are supported in addition to coil-per-cylinder direct fire. Rare "dual distributor" mode is also supported for engines like older BMW V12s and the Toyota 1UZ.
...
Unipolar stepper valves are not supported by any hardware variants. Chrysler commonly used these.
...
Relay Should tick even when your ECU is out!

**HOWEVER** if your car is switched by a ECM pin, take a free LS (low side) output and replace your old relay operation signal with it,
...
Switch goes (ON) Position > ECU Gets power > ECU sends a ground Signal to EFI Relay > you get power to EFI components!
...
VR sensors typically have 2 wires and this type of waveform:
...
Hall effect sensors have a Square shaped wave and typically a 3 wire system
...
Analog inputs are broken into two basic categories:
