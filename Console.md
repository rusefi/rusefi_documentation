...
rusEFI Console is a handy development and debugging tool. It is written in java and talks to the ECU firmware. The console allows firmware monitoring and configuration. Here are some pictures of what it looks like when it is operating correctly.
...
![Jumper Setup](Images/rusEFI_STM.jpg)
...
* When the driver is installed & the board is connected via the micro USB cable, you can verify serial connectivity by
  ...
  * '''Linux'''
    ...
    * In this case, the newly plugged in USB is registered to ttyACM0, which you can find on /dev/ttyACM0
...
* Note: rusEFI Console requires Java 1.8 or later!
* When you launch the Console, it will ask you to select which serial address it should use. However you can specify that when you launch it by specifying a command line parameter as noted below.
  * Windows
    `java -jar rusefi_console.jar COM15`
  * Linux (ubuntu 11.10)
    `java -jar rusefi_console.jar /dev/ttyACM0`
* You got it to open, great! Now install the gray jumper wire as noted above between pins PD1 and PC6.
* Select "digital sniffer" in rusEFI Console and you should see the simulated signal.
