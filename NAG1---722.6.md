...
The TCM continuously checks for electrical problems, mechanical problems and some hydraulic problems. When a problem is sensed, the TCM stores a Diagnostic Trouble Code (DTC). Some of these codes cause the transmission to go into “Limp-In” or “Default” mode.
The NAG 1 transmission defaults in the current gear position if a DTC is detected.
Then after a key cycle or if the transmission is placed in Park, the transmission goes into Limp-In Mode, which is mechanical second gear. Some DTCs allow the transmission to resume normal operation (recover) if the detected problem goes away.
A Permanent Limp-In DTC recovers when the key is cycled; but if the same DTC is detected for three key cycles, the system does not recover and the DTC must be cleared from the TCM with a scan tool.
